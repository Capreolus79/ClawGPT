...
Example: [0, 2, 5]`;
  ...
  renderMessages() {
    ...
    const visibleMessages = chat.messages.filter(msg => msg.content && msg.content.trim());
    
    this.elements.messages.innerHTML = visibleMessages.map((msg, idx) => {
      const isUser = msg.role === 'user';
      const isLastAssistant = !isUser && idx === visibleMessages.length - 1;
